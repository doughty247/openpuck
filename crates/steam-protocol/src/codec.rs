//! Input translation layer: BLE/USB report bytes <-> `GamepadState` <->
//! virtual/USB HID output report bytes.
//!
//! This is the single implementation shared by the ESP32-S3 firmware
//! (`src/controller.rs` re-exports it) and the `freepuck-software` desktop
//! bridge (`tools/freepuck-software/src/controller.rs` re-exports it too).
//! It used to be two independently-maintained copies; cross-referencing an
//! independent reverse-engineering of the same controller
//! ([safijari/openpuck](https://github.com/safijari/openpuck)) found real
//! bugs that existed in both copies (see `parse_steam`'s doc comment) —
//! exactly the failure mode a single shared implementation exists to
...
pub struct GamepadState {
    ...
    pub pad_y: u16,
    pub pad_active: bool,
    // IMU motion data from the Steam Controller 2 (raw i16, signed, 0 = no data).
    pub gyro_x: i16, pub gyro_y: i16, pub gyro_z: i16,
    pub accel_x: i16, pub accel_y: i16, pub accel_z: i16,
}

/// Normalized haptics intent derived from a USB/virtual-device rumble callback.
///
...
impl GamepadState {
    ...
    /// state to present before the first real input report arrives.
...
pub fn dpad_to_hat(up: bool, down: bool, left: bool, right: bool) -> u8 {
...
// ------------------------------------------------------------------
// Input parsers  (input report bytes -> GamepadState)
// ------------------------------------------------------------------

/// Steam Controller 2 full-state input report. payload[0]=seq; btn bytes
/// 1-4; triggers 5-8; sticks 9-16; right pad, gyro, and accel ride in the
/// tail of a full report (see below) and are only present on 45+ byte
/// notifications, not the 17-byte minimum.
///
/// This offset scheme is BLE-side: on the firmware's BLE GATT input
/// characteristic, the notification payload appears to be the controller's
/// native report 0x45 with its leading report-ID byte stripped (every
/// field lines up exactly one byte earlier than the raw-report offsets
/// documented below). The firmware's USB-puck path (`puck.rs`, behind the
/// `rf` feature) reads the same fields from the *raw* 46-byte report
/// (report ID included, no shift) and calls this function directly at
/// those native offsets — see that module for how it adapts.
///
...
/// report 0x45 is 46 bytes starting with the report ID. That cross-check
/// is what caught three real bugs (see below) that existed in both this
/// crate's previous BLE-side copy and the firmware's copy before this
/// crate existed:
///
/// 1. Touchpad click read byte 3 (`btn2`) bit 1 — actually the L4
///    back-paddle button, not a touchpad bit at all.
/// 2. Trigger scaling wrapped instead of saturating on a hard pull
///    (`raw >> 7` can reach 511; a plain `as u8` cast wraps that back down
///    to a small value instead of reading as fully pressed).
/// 3. Digital trigger-press was a pure analog-threshold guess (`raw >
///    8000`) instead of using the controller's own dedicated digital-click
///    bit per trigger.
///
/// The BLE/report-ID-stripped alignment itself is *inferred* from a
/// differently-transported reference implementation, not confirmed against
/// a real BLE capture — treat the fields that only exist past byte 16
/// (real trackpad coordinates, gyro, accel) and the two digital
...
pub fn parse_steam(payload: &[u8]) -> Option<GamepadState> {
    ...
    // Always present -- it falls inside the 17-byte minimum, we just
    // weren't reading it before this field was mapped.
    let btn3 = payload[4];
...
// ------------------------------------------------------------------
// Output formatters  (GamepadState -> USB/virtual HID report bytes)
// ------------------------------------------------------------------

/// XInput (Xbox 360) report — 20 bytes. Matches the layout used by the
/// firmware's USB gadget and by ViGEmBus / vigem-client on Windows.
...
pub fn state_to_dualsense(s: &GamepadState, steam_touchpad_mode: bool) -> [u8; 64] {
    ...
    // r[16..22]= Gyro X/Y/Z (i16 LE)
    // r[22..28]= Accel X/Y/Z (i16 LE)
    ...
    // Gyro X/Y/Z at r[16..22] (signed i16 LE).
    ...
    // is why any scale factor here would be an unconfirmed guess — "no
    // scaling," matching a project others have actually run, is the safer
